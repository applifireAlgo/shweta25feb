Ext.define('Mobile1aprl.view.smartdevice.login.LoginController', {
	extend : 'Ext.app.ViewController',
	alias : 'controller.login',
	//views : [ "Login" ],
	requires : ['Mobile1aprl.view.smartdevice.design.drawer.main.MainPanel',
	'Mobile1aprl.view.smartdevice.login.ForgetPassword'],
...
/*	,onSessionLoginClick : function() {
		...
		if (form.isValid()) {
			...
			Ext.Ajax.request({
				url : "http://digitization.cloudapp.net:8080/Mobile1aprl/secure/Authentication/authenticate",
				method : 'POST',
						...
						if (jsonResponse.response.changePassword == true) {
							Ext.Msg.confirm('Confirm', jsonResponse.response.message, function(id, value) {
								if (id == 'yes') {
									...
									scope.container.removeAll();
									scope.container.add(Ext.create('Mobile1aprl.view.smartdevice.login.ChangePasswordScreen'));
								}
							}, {
								controller : this,
								scope : scope
	...
	onForgetPasswordClick : function(btn, opts) {
		...
		Ext.Ajax.request({
			timeout : 180000,
			url : "http://digitization.cloudapp.net:8080/Mobile1aprl/secure/PasswordGenerator/findSecurityQuestions",
			...
			success : function(response, sender) {
				...
				if (responseText.response.success) {
					...
					var forgetPasswordWindow = Ext.create('Mobile1aprl.view.smartdevice.login.ForgetPasswordWindow', {
