Ext.define('Mobile1aprl.view.smartdevice.login.ForgetPasswordController',
		{
				...
				if (this.getView().down('#secQuestion').isValid()
					...
					Ext.Ajax.request({
						...
						success : function(response, sender) {
							...
							container.removeAll();
							container.add(Ext.create('Mobile1aprl.view.smartdevice.login.Login'));
							sender.entMask.hide();
						},
			...
			onCancelClick : function(btn, opts) {
				...
				container.removeAll();
				container.add(Ext
						.create('Mobile1aprl.view.smartdevice.login.Login'));
			}
});