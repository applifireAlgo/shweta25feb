Ext.define('Salespro1.view.mobileview.login.LoginController', {
			...
			Ext.Ajax.request({
				...
				success : function(response, scope) {
					...
					if (jsonResponse.response.success) {
						...
						scope.container.add(Ext.create('Salespro1.view.mobileview.main.MainPanel'));
					} else {
						if (jsonResponse.response.changePassword == true) {
							Ext.Msg.confirm('Confirm', jsonResponse.response.message, function(id, value) {
								if (id == 'yes') {
									...
									scope.container.removeAll();
									scope.container.add(Ext.create('Salespro1.view.mobileview.login.ChangePasswordScreen'));
								}
							}, {
								controller : this,
								scope : scope
		...
		Ext.Ajax.request({
			...
			success : function(response, sender) {
				...
				if (responseText.response.success) {
					...
					var forgetPasswordWindow = Ext.create('Salespro1.view.mobileview.login.ForgetPasswordWindow', {
